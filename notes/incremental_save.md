...
Incremental save applies **only** to *data mode*. It allows the plugin to write new rows to disk during long‑running loops, reducing data loss if the program crashes.
...
- **Case B: `saved >= meta_rows`**  
  Append only the **new data rows**, i.e. rows from index `saved` up to `data_rows - 1`, provided there is at least one new row.
