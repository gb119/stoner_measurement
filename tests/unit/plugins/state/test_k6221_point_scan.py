...
@pytest.mark.parametrize(
    "primary,secondary", [(False, False), (True, False), (False, True), (True, True)]
)
def test_independent_meters_and_output_schema(connected, primary, secondary):
    ...
    # Exercise expressions generated by this plugin, not external input.
    values = {
        key.split(".")[-1]: eval(expression, {plugin.instance_name: plugin})  # noqa: S307  # nosec B307
...
def test_repeated_points_hold_dc_without_running_hardware_sweeps(connected):
    ...
    assert source.mock_calls == [
        call.set_source_level(0.001),
        call.enable_output(True),
        call.set_source_level(0.002),
        call.enable_output(True),
    ]
