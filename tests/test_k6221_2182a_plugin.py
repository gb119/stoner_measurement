...
def _make_plugin(qapp) -> Keithley6221_2182APlugin:
...
class TestIdentity:
    def test_name(self, qapp):
        assert _make_plugin(qapp).name == "Keithley6221_2182A"

    def test_trace_title(self, qapp):
        assert _make_plugin(qapp).trace_title == "6221/2182A I-V"

    def test_x_label(self, qapp):
        assert _make_plugin(qapp).x_label == "I"

    def test_y_label(self, qapp):
        assert _make_plugin(qapp).y_label == "V"

    def test_x_units(self, qapp):
        assert _make_plugin(qapp).x_units == "A"

    def test_y_units(self, qapp):
        assert _make_plugin(qapp).y_units == "V"

    def test_plugin_type(self, qapp):
        assert _make_plugin(qapp).plugin_type == "trace"

    def test_num_traces(self, qapp):
        assert _make_plugin(qapp).num_traces == 1
...
class TestDefaults:
    def test_default_connection_mode(self, qapp):
        assert _make_plugin(qapp)._connection_mode is ConnectionMode.VIA_6221_SERIAL

    def test_default_scan_generator_type(self, qapp):
        assert isinstance(_make_plugin(qapp).scan_generator, SteppedScanGenerator)

    def test_scan_generator_units_are_amps(self, qapp):
        assert _make_plugin(qapp).scan_generator.units == "A"

    def test_default_compliance(self, qapp):
        assert _make_plugin(qapp)._compliance == pytest.approx(10.0)

    def test_default_nplc(self, qapp):
        assert _make_plugin(qapp)._nplc == pytest.approx(1.0)

    def test_default_output_tlink(self, qapp):
        assert _make_plugin(qapp)._output_tlink == 1

    def test_default_input_tlink(self, qapp):
        assert _make_plugin(qapp)._input_tlink == 2

    def test_no_active_instruments_at_init(self, qapp):
        plugin = _make_plugin(qapp)
        assert plugin._k6221 is None
        assert plugin._k2182a is None

    def test_initial_status_is_idle(self, qapp):
        assert _make_plugin(qapp).status is TraceStatus.IDLE
...
class TestJsonRoundTrip:
    ...
    def test_restore_unknown_connection_mode_falls_back(self, qapp):
        """An unknown connection_mode value in saved JSON must not raise."""
        plugin = _make_plugin(qapp)
        data = plugin.to_json()
        data["connection_mode"] = "future_mode_value"

        with pytest.warns(None):  # no warnings required, but must not raise
            try:
