"""Helpers for loading motor-controller engine configuration."""
...
def machine_config_path() -> Path:
    """Return the per-machine motor-controller config path."""
    return user_config_file("motor_controller.yaml")


def load_motor_controller_config() -> dict[str, Any]:
    """Load merged bundled and per-machine engine configuration."""
    bundled = load_yaml_mapping(
        bundled_resource_path("", "motor_controller.yaml") or Path("__missing__")
    )
    machine = load_yaml_mapping(machine_config_path())
    return deep_merge(bundled, machine)


def save_motor_controller_config(config: dict[str, Any]) -> Path:
    """Save machine-specific motor-controller configuration."""
